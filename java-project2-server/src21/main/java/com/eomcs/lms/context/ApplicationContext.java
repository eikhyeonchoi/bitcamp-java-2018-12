...
public class ApplicationContext {
  ...
  public ApplicationContext(Class<?> configClass) throws Exception {
    //l IoC 컨테이너와 관련된 설정 정보를 갖고 있는 클래스 정보를 받아서 초기화를 수행한다.
    
    //l 1) 설정 정보를 갖고 있는 클래스의 인스턴스를 생성한다.
    Constructor<?> c = configClass.getConstructor();
    Object config = c.newInstance();
    
    //l 2) Bean 애노테이션이 붙은 메서드를 모두 찾는다.
    ...
    //l 3) 팩토리 메서드를 호출하여 그 리턴 값을 빈 컨테이너에 보관한다.
    while (factoryMethods.size() > 0) {
      Method m = factoryMethods.get(0); //l 메서드 목록에서 메서드를 꺼내
      callFactoryMethod(config, m, factoryMethods, ""); //l 호출한다.
      factoryMethods.remove(m); //l 호출에 성공하든 실패하든 목록에서 제거한다.
    }
    
    
    //l 4) ComponentScan 애노테이션을 처리한다
    //l    ==> 애노테이션에 지정된 패키지를 탐색하여 객체를 생성
  ...
  public void prepareComponent(String packageName) throws Exception {
    ...
    //l 2) 해당 패키지 폴더와 그 하위 폴더를 뒤져서 클래스 이름을 알아낸다.
    //l => 인스턴스를 생성할 수 없는 인터페이스나 추상 클래스는 제외한다.
    //l => 또한 중첩 클래스도 제외한다.
    findClasses(packageDir, packageName);
    
    //l 3) Component 애노테이션이 붙은 클래스만 찾아서 인스턴스를 생성한다.
    createComponent();
  ...
  private void findClasses(File dir, String packageName) throws Exception {
    //l 디렉토리를 뒤져서 클래스 파일(.class)이나 하위 디렉토리 목록을 알아낸다.
    ...
    for (File f : files) {
      if (f.isFile()) {
        //l 클래스 파일일 경우,
        //l => 파라미터로 받은 패키지 명과 파일 이름을 합쳐서 클래스 이름을 만든다.
        //l    예) com.eomcs.lms(패키지명) + . + ServerApp(파일명) = com.eomcs.lms.ServerApp
        String filename = f.getName();
        String className = packageName + "." + 
            filename.substring(0, filename.indexOf('.'));
        
        //l => 클래스 이름으로 클래스 파일(.class)을 로딩한다.
        Class<?> clazz = Class.forName(className);
        
        //l => 클래스 정보를 분석하여 중첩 클래스이거나 인터페이스, Enum 이면 무시한다.
        if (clazz.isLocalClass() || clazz.isInterface() || clazz.isEnum())
          continue;
        
        //l => 추상 클래스나 공개되지 않은 클래스(public이 아닌 클래스)도 무시한다.
        if (Modifier.isAbstract(clazz.getModifiers()) ||
            !Modifier.isPublic(clazz.getModifiers()))
          continue;
        
        //l 즉 공개된(public) 일반 클래스인 경우 클래스 관리 목록에 추가한다. 
        classes.add(clazz);
        
      } else {
        //l 디렉토리일 경우, 그 하위 디렉토리에서 다시 클래스를 찾는다.
        //l => 하위 디렉토리에서 클래스를 찾을 때 사용할 패키지명을 준비한다.
        //l => 파라미터로 받은 패키지 이름에 하위 디렉토리 이름을 붙이면 전체 패키지명이 된다.
        //l => com.eomcs.lms(현재 패키지 이름) + . + dao(디렉토리 이름) = com.eomcs.lms.dao
  ...
  private void createComponent() throws Exception {
    for (Class<?> clazz : classes) {
      //l 클래스에서 Component 애노테이션 정보를 추출한다.
      ...
      if (obj != null) { //l 제대로 생성했으면 빈컨테이너에 저장한다.
        //l 빈컨테이너에 객체를 저장할 때 key 값은 Component 애노테이션의 value() 값으로 한다.
        //l 만약 value 가 빈 문자열이라면 클래스 이름을 사용한다.
  ...
  private Object createInstance(Class<?> clazz) throws Exception {
    ...
    try { 
      ...
      //l 기본 생성자를 못 찾으면 예외 발생한다. 
      //l 그냥 무시하고 다른 생성자를 찾아 인스턴스를 생성한다.
    }
    
    //l => 기본 생성자가 없다면, 다른 생성자 목록을 얻는다.
    Constructor<?>[] constructors = clazz.getConstructors();
    for (Constructor<?> c : constructors) {
      //l => 생성자를 호출하려면 먼저 어떤 타입의 파라미터가 필요한지 알아야 한다.
      Class<?>[] paramTypes = c.getParameterTypes();
      
      //l => 생성자가 요구하는 타입의 파라미터 값이 저장소에 있는지 찾아 본다.
      Object[] paramValues = getParameterValues(paramTypes);
      if (paramValues != null) { //l 생성자가 요구하는 모든 파라미터 값을 찾았다면 
        //l 생성자를 통해 인스턴스를 생성하여 리턴한다.
  ...
  private Object[] getParameterValues(Class<?>[] paramTypes) {
    //l 파라미터 타입에 해당하는 객체를 빈컨테이너에서 찾아 배열을 만들어 리턴한다.
    ...
    if (values.size() == paramTypes.length) 
      //l 파라미터의 타입 목록에 지정된 객체를 모두 찾았으면 배열로 리턴한다.  
      return values.toArray();
    else //l 못 찾았으면 null을 리턴한다.
      return null;
  }
  
  private Object findBean(Class<?> type) {
    //l 빈 컨테이너에서 특정 타입의 인스턴스를 찾기
  ...
  public void postProcess() {
    ...
    for (Object bean : beans) {
      //l 각 객체에 대해 @RequestMapping 메서드를 찾는다.
    ...
    //l ServerApp에서 꺼낼 수 있도록 RequestMappingHandlerMapping 객체를 
    //l 빈 컨테이너에 저장해 둔다.
...
//    System.out.println(indent + "==>" + factoryMethod.getName());
    ...
    //l 1) m 메서드에서 호출할 때 사용할 파라미터 타입 정보를 알아낸다.
    //l    ex) m(BoardDao, MemberDao, TransactionManager)
    //l        파라미터 타입 배열 : 
    //l        {BoardDao.class, MemberDao.class, TransactionManager.class} 
    Class<?>[] paramTypes = factoryMethod.getParameterTypes(); 
    
    //l 파라미터 값을 담을 배열을 준비한다.
    //l ex) {new BoardDao(), new MemberDao(), new TranscationManager()}
    Object[] paramValues = new Object[paramTypes.length];
    
    for (int i = 0; i < paramTypes.length; i++) {
      //l 2) 빈 컨테이너에서 파라미터 타입에 해당하는 값을 꺼낸다.
      ...
      //l 3) 만약 빈 컨테이너에 파라미터 타입에 해당하는 값이 없다면, 
      //l    팩토리 메서드를 뒤져서 그 타입의 값을 리턴하는 메서드를 찾아 호출한다.
      Method otherFactoryMethod = findMethodByReturnType(
          factoryMethods, paramTypes[i]);
      if (otherFactoryMethod == null) {
        //l 4) 만약 해당 파라미터 타입의 값을 만들어주는 다른 팩토리 메서드가 없다면 
        //l    factoryMethod 호출을 포기한다.
        return null;
      }
      
      //l 파라미터 값을 만들어 줄 다른 팩토리 메서드를 찾았다면, 그 메서드를 호출한다.
      //l => 재귀 호출을 사용한다.
      paramValue = callFactoryMethod(obj, otherFactoryMethod, factoryMethods, indent + "    ");
      if (paramValue == null) {
        //l 파라미터 값을 리턴해주는 팩토리 메서드를 찾긴 했지만 
        //l 그 메서드를 호출하기 위해 파라미터 값이 필요했는데,
        //l 그 파라미터 값이 없어서 메서드 호출에 실패했다면
        //l factoryMethod 호출을 포기한다.
        return null;
      }
      
      //l 파라미터 값이 준비되었으면 저장한다.
      paramValues[i] = paramValue;
    }
    
    //l 팩토리 메서드를 호출할 수 있다면 팩토리 메서드 목록에서 제거한다.
    factoryMethods.remove(factoryMethod);

    //l 준비한 파라미터 값들을 가지고 팩토리 메서드를 호출한다.
    //l 그리고 팩토리 메서드가 생성한 객체를 빈 컨테이너에 보관한다.
    Object bean = factoryMethod.invoke(obj, paramValues);
    
    
    //l 팩토리 메서드가 생성한 객체를 저장할 때,
    //l 애노테이션에 빈의 이름이 지정되어 있다면 그 이름을 사용하고 
    //l 없다면 메서드 이름을 사용한다.
