...
//     ==> 예) BoardService의 이름을 BoardDaoSkel로 변경
// 4) 클라이언트쪽 BoardDaoImpl 객체를 대해앟ㄹ 프록시 클래스를 만든다  
//     ==> 예) BoardAgent 클래스를 BoardDaoProxy클래스로 변경
//           BoardProxy 클래스는 BoardImpl 클래스와 마찬가지로 BoardDao 인터페이스 구현
//             패키지명도 agent대신 proxy로,,
