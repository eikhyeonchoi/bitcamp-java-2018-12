  ...
  public static void main(String[] args) {
    ...
    try ( Scanner keyboard = new Scanner(System.in);
      ...
      try (Socket socket = serverSocket.accept();
        ...
        System.out.println("server : 클라이언트와 연결 완료 ...");

        // 클라이언트가 보낼 데이터의 크기와 파일 이름을 읽음
        ...
        try (BufferedOutputStream fileOut = new BufferedOutputStream(
          ...
          System.out.println("server : 파일을 받는중 ...");
          for(long i = 0; i < fileLen; i++) {
            fileOut.write(in.read()); // 소켓에서 읽은 바이트를 버퍼에 저장한다
            // 버퍼가 모두 차면 자동으로 파일에 출력할 것이다
          }
          // 버퍼에 남은 데이터를 마저 출력해야 한다
          fileOut.flush();
          System.out.println("server : 파일 다운로드 완료 ...");
          out.println("success ...");
          out.flush();
        ...
        System.out.println("server : 클라이언트에게 응답했음 ...");
