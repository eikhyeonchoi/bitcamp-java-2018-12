...
public class Test05 {
    ...
    IO Stream 클래스의 역할에 따른 분류
      ...
      2) date processing stream class
          ==> 데이터 읽고 쓸 때 중간에서 데이터를 가공하는 클래스
          ==> 데코레이터 역할을 수행한다
     ...
     바이너리 스트림과 문자 스트림 클래스
       1) binary stream class
         ...
         ==> InputStream/OutputStream의 서브클래스이다
           ==> 보통 클래스이름이 InputStream/OutputStream으로 끝난다
           
       2) character stream class
           ==> 문자 단위로 데이터를 다루는 클래스
           ==> 데이터를 입출력할 때 UTF-16을 다른 문자표의 코드 값으로 자동 변환 해준다
