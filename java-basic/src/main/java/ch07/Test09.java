...
// 패키지 클래스
// ==> 단독으로 선언하는 클래스
// ==> 물론 이 예제 처럼 한 파일에 여러 개의 클래스를 선언할 수 있지만,
...
public class Test09 {
  ...
  // ==> 이 클래스를 소유 하는 클래스 뿐만 아니라 다른 클래스도 사용 가능할 수 있다.
  static class Score2 {
    String name;
    int kor;
    int eng;
  ...
  // ==> 클래스 이름이 없는 중첩 클래스
  // ==> 딱 한개의 인스턴스를 생성할 때 사용\
  // ==> 클래스를 정의할 때 수퍼 클래스나 인터페이스를 지정해야함
  // ==> 클래스를 정의할 때 new 연산자를 사용해 즉시 인스턴스를 생성한다
  ...
  void m2() {
    ...
    // 논-스태틱 멤버는 스태틱 멤버를 사용할 수 있다.
    Score2 s2 = new Score2();

    // 인스턴스 멤버는 인스턴스 멤버를 사용할 수 있다
    Score3 s3 = new Score3();
