...
public class Test03 {
  ...
  //              변수 선언;
  //         }
  // - 새데이터 타입의 이름 : 대문자로 시작하고, 단어의 시작도 대문자,보통 명사형
  static class Score {
  ...
  public static void main(String[] args) {
    ...
    s1 = new Score(); // Score 설계도에 따라 메모리(변수들)을 준비시킨다.
                         // 그리고 그 메모리의 주소를 레퍼런스 s1에 저장한다
    
    // 물론 다음과 같이 한 줄에 표현할 수 있다
    Score s2 = new Score();
    
    // 새 데이터 타입에 따라 만든 메모리
    // ==> Score클래스의 인스턴스에 값 넣기
    //  ==> 문법:
    //          레퍼런스.변수명 = 값;
    s1.name = "홍길동";
    s1.kor = 100;
    ...
    printScore(s1); // 이전 예제처럼 여러 개의 값을 넘겨줄 필요x 편리함
    printScore(s2);
    
    final int LENGTH = 10;
    Score[] scoreInfo = new Score[LENGTH];
    scoreInfo[0].name = "김민철";
