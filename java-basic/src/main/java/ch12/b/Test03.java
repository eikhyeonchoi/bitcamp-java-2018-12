...
public class Test03 {
  ...
  public static void main(String[] args) {
    ...
    // 이유 1)
    // 만약 국, 영, 수 점수를 변경한 후 compute()를 호출하지 않고 평균과 총점을 출력
    // 분명 국어점수를 변경했음에도 총점과 평균이 바뀌지 않는다
    // 개발자 compute()를 호출하는 것을 빠뜨리면 이런 문제 발생
    s1.kor = 70;
    System.out.printf("총점 : %d, 평균 : %.2f\n", s1.getSum(), s1.getAver());
    ...
    // 제대로 계산까지 한다
    s1.compute();
    // 이번에는 점수를 변경한 후 compute()를 호출하여 계산까지 했지만,
