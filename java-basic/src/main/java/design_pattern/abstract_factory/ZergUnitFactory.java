...
//     서브 클래스에게 상속해줄 필드나 메서드가 있다면 중간에 추상클래스 문법을 적용하여
//     추상 클래스를 만드는 것이 좋다
// ==> 서브 클래스에게 상속 해줄 필드나 메서드가 없다면 직접 인터페이스를 구현
public class ZergUnitFactory implements UnitFactory{
  ...
  private Unit createRestaurant() {
    ...
    unit.setArea(70);
    unit.setType(Restaurant.DEFENCE_BUILDING);
    return unit;
  }
  private Unit createTrainingCenter() {
    ...
    unit.setArea(300);
    unit.setType(Restaurant.ATTACK_BUILDING);
    return unit;
  }
