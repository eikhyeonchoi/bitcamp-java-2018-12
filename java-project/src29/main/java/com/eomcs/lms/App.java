  ...
  private static void loadLessonData() {
    try(DataInputStream in = 
      ...
      for(int i = 0; i < len; i ++) {
        ...
        les.setNo(in.readInt());
        les.setTitle(in.readUTF());
        les.setContents(in.readUTF());
        les.setStartDate(Date.valueOf(in.readUTF()));
        les.setEndDate(Date.valueOf(in.readUTF()));
        les.setTotalHours(in.readInt());
        les.setDayHours(in.readInt());
        lessonList.add(les);
      }
    } catch (Exception e) {
